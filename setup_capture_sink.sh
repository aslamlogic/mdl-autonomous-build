#!/usr/bin/env bash
set -euo pipefail
# setup_capture_sink.sh
# Usage:
#   paste this whole block into your terminal and run:
#     bash setup_capture_sink.sh
# To auto-push to origin after commit set AUTO_PUSH=1 in environment:
#   AUTO_PUSH=1 bash setup_capture_sink.sh

ROOT_DIR="$(pwd)"
LOG_DIR="logs"
WORKFLOW_DIR=".github/workflows"
WORKFLOW_FILE="$WORKFLOW_DIR/capture-tests.yml"
CAPTURE_PY="capture_sink.py"
GIT_COMMIT_MSG="chore: add capture_sink and CI workflow for automated error/solution capture"
...
jobs:
  test-and-capture:
    ...
    steps:
      ...
      - name: Run tests via capture sink (captures failures to logs/errors.log)
        run: |
          python capture_sink.py run --component tests --severity high -- pytest -q
        continue-on-error: true

      - name: Show last error lines (for quick debug)
        run: tail -n 50 logs/errors.log || true
...
if ! grep -q "^logs/" .gitignore 2>/dev/null; then
  echo "logs/" >> .gitignore || true
fi

# Makefile: add test-capture if not present
if [ -f Makefile ]; then
  if ! grep -q "^test-capture:" Makefile; then
    cat >> Makefile <<'MK'
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  git init
  branch="main"
  echo "Initialized a new git repo (no remote)."
fi

git add "$CAPTURE_PY" "$WORKFLOW_FILE" .gitignore Makefile || true
...
if [ "${AUTO_PUSH:-0}" = "1" ]; then
  if git remote | grep -q origin; then
    ...
    git push origin "$branch" || echo "git push failed (check credentials/remote)."
  else
    echo "No 'origin' remote found. Skipping push."
  fi
else
  echo "AUTO_PUSH not set to 1; skipping git push. To push automatically set AUTO_PUSH=1."
fi

echo
echo "Setup complete."
echo " - capture sink: $CAPTURE_PY (executable)"
echo " - workflow: $WORKFLOW_FILE"
echo " - logs dir: $LOG_DIR/ (ignored by .gitignore)"
echo
echo "To run tests locally via the capture sink:"
echo "  make test-capture"
echo
echo "If you want the CI to run immediately, push to your repo (or set AUTO_PUSH=1 to let this script push)."
